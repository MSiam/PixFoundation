...
class CustomCOCODataset(Dataset):
    ...
    def __init__(self, mean, std, prefix='mmvp_val', varidx=-1):
        self.mean = mean
        self.std = std
        self.normalize = True
        if prefix != 'mmvp_val':
            assert varidx != -1, "Prompt sensitivity datasets need variation index"

        dataset_dicts = [DatasetCatalog.get(prefix)]
