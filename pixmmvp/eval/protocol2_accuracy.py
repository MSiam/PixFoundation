...
def compute_metrics(jsonl_file, output_file, csv_file, extra_outdir=None):
    ...
    dataframe = pd.DataFrame(finegrained_correct, columns=['acc'])
    dataframe.to_csv(args.finegrained_csv_file)
    combined_data = {
        "model": model,
        "time": time_string,
        "total": total,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--finegrained_csv_file", type=str, default="./finegrained_acc.csv", help="Path to the output csv file to store the experiment data")
